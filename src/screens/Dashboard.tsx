...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchEmployeeAndClient = async () => {
      ...
      if (!user || !user.id) {
        console.error(
          "No user data found, skipping employee and client fetch.",
        );
        return;
      }
  ...
  const fairSurveyCount =
    surveyCount - LockedRoomSurveyCount - deniedSurveyCount;
  return (
    <ScrollView
      refreshControl={
    ...
    >
      <View className="flex-1">
        ...
        <View className="flex items-start justify-center bg-white border border-slate-200 m-4 rounded-xl">
          ...
          <TouchableOpacity
            onPress={() => {
              navigation.navigate("Select Search Option");
            }}
          >
            <View className="h-10 m-2 w-full flex flex-row items-center justify-evenly">
              {/* <AntDesign name="edit" size={24} color="blue" className="w-1/4" /> */}
