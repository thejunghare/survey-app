...
interface SurveyContextProps {
    ...
    countDocument: (userId: string) => Promise<string>;
}
...
export function SurveyProvider({children}: SurveyProviderProps) {
    ...
    async function countDocument(userId: string) {
        const response = await databases.listDocuments(SURVEY_DATABASE_ID, SURVEY_COLLECTION_ID, [
    ...
    return (
        <SurveyContext.Provider value={{current: surveys, add, remove, countDocument}}>
            {children}
